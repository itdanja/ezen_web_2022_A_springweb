...
//    //1. 현재 내 위치의 위도 경도 구하기 [ 배포시 https 에서만 사용가능 ]
...
//    }); // 현재 내 위치의 위도경도 구하기 end
    ...
    // 4. 마커 이미지 변경
    var markerImageUrl = 'http://localhost:8081/img/icon_home.png', // 마커 이미지의 주소
...
function getmap(){
                ...
                $.ajax({
                    url: '/room/roomlist' ,
                    data : JSON.stringify(  this.searchobject ) , //  검색옵션 + 현재 보고 있는 지도 좌표
...
function changeprice(){
    ...
    if( minprice > maxprice ){ // 만약 최소금액 더 크면
        alert('최소금액이 더 큽니다.');
   ...
   $("#pricebox").html( minpricemillion+" ~ "+maxpricemillion );
